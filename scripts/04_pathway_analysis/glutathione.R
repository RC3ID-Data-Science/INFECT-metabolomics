...
glu_pathway <- keggGet("hsa00480")

glu_compounds <- names(glu_pathway[[1]]$COMPOUND)

glu_hmdb = read.csv("name_map_glu.csv")

matching_cols <- colnames(merge)[colnames(merge) %in% glu_hmdb$HMDB]
...
merge_glu_renamed <- merge_glu

metabolite_cols <- glu_hmdb$Match[match(names(merge_glu_renamed)[names(merge_glu_renamed) %in% glu_hmdb$HMDB], 
                                       glu_hmdb$HMDB)]

names(merge_glu_renamed)[names(merge_glu_renamed) %in% glu_hmdb$HMDB] <- 
  glu_hmdb$Match[match(names(merge_glu_renamed)[names(merge_glu_renamed) %in% glu_hmdb$HMDB], 
                      glu_hmdb$HMDB)]
...
kegg_id_map <- glu_hmdb |> 
  filter(glu_hmdb$Match %in% l2fc_data$Metabolite)
