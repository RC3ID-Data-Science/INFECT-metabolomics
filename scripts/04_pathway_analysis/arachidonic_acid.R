...
aa_pathway <- keggGet("hsa00590")

aa_compounds <- names(aa_pathway[[1]]$COMPOUND)

aa_hmdb = read.csv("name_map_aa.csv")

matching_cols <- colnames(merge)[colnames(merge) %in% aa_hmdb$HMDB]
...
merge_aa_renamed <- merge_aa

metabolite_cols <- aa_hmdb$Match[match(names(merge_aa_renamed)[names(merge_aa_renamed) %in% aa_hmdb$HMDB], 
                                        aa_hmdb$HMDB)]

names(merge_aa_renamed)[names(merge_aa_renamed) %in% aa_hmdb$HMDB] <- 
  aa_hmdb$Match[match(names(merge_aa_renamed)[names(merge_aa_renamed) %in% aa_hmdb$HMDB], 
                       aa_hmdb$HMDB)]
...
kegg_id_map <- aa_hmdb |> 
  filter(aa_hmdb$Match %in% l2fc_data$Metabolite)
