...
try_pathway <- keggGet("hsa00380")

try_compounds <- names(try_pathway[[1]]$COMPOUND)
try_hmdb = read.csv("name_map_try.csv")

matching_cols <- colnames(merge)[colnames(merge) %in% try_hmdb$HMDB]
...
merge_try_renamed <- merge_try

metabolite_cols <- try_hmdb$Match[match(names(merge_try_renamed)[names(merge_try_renamed) %in% try_hmdb$HMDB], 
                                        try_hmdb$HMDB)]

names(merge_try_renamed)[names(merge_try_renamed) %in% try_hmdb$HMDB] <- 
  try_hmdb$Match[match(names(merge_try_renamed)[names(merge_try_renamed) %in% try_hmdb$HMDB], 
                       try_hmdb$HMDB)]
...
kegg_id_map <- try_hmdb |> 
  filter(try_hmdb$Match %in% l2fc_data$Metabolite)
