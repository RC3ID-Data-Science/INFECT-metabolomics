...
pro_pathway <- keggGet("hsa00330")

pro_compounds <- names(pro_pathway[[1]]$COMPOUND)

pro_hmdb = read.csv("name_map_pro.csv")

matching_cols <- colnames(merge)[colnames(merge) %in% pro_hmdb$HMDB]
...
merge_pro_renamed <- merge_pro

metabolite_cols <- pro_hmdb$Match[match(names(merge_pro_renamed)[names(merge_pro_renamed) %in% pro_hmdb$HMDB], 
                     pro_hmdb$HMDB)]

names(merge_pro_renamed)[names(merge_pro_renamed) %in% pro_hmdb$HMDB] <- 
  pro_hmdb$Match[match(names(merge_pro_renamed)[names(merge_pro_renamed) %in% pro_hmdb$HMDB], 
                      pro_hmdb$HMDB)]
...
kegg_id_map <- pro_hmdb |> 
  filter(pro_hmdb$Match %in% l2fc_data$Metabolite)
