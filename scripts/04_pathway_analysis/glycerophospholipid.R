...
pho_pathway <- keggGet("hsa00564")

pho_compounds <- names(pho_pathway[[1]]$COMPOUND)
write.csv(pho_compounds,"pho_compounds.csv")
pho_hmdb = read.csv("name_map_pho.csv")

matching_cols <- colnames(merge)[colnames(merge) %in% pho_hmdb$HMDB]
...
merge_pho_renamed <- merge_pho

metabolite_cols <- pho_hmdb$Match[match(names(merge_pho_renamed)[names(merge_pho_renamed) %in% pho_hmdb$HMDB], 
                                        pho_hmdb$HMDB)]

names(merge_pho_renamed)[names(merge_pho_renamed) %in% pho_hmdb$HMDB] <- 
  pho_hmdb$Match[match(names(merge_pho_renamed)[names(merge_pho_renamed) %in% pho_hmdb$HMDB], 
                       pho_hmdb$HMDB)]
...
kegg_id_map <- pho_hmdb |> 
  filter(pho_hmdb$Match %in% l2fc_data$Metabolite)
